...
class StarApp(tk.Tk):
    ...
    def on_segment(self):
        if not self.field.stars:
            messagebox.showwarning("Пусто", "Сначала загрузите данные.")
            return
        try:
            ...
            messagebox.showerror("Ошибка", "Порог расстояния должен быть положительным числом.")
            return
    ...
    def show_cluster_pie_chart(self):
        ...
        counts = Counter(self.field.cluster_ids)
